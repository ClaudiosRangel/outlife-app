-- Loja virtual — banner chamativo: tema (paleta de cores) e layout escolhidos
-- na criação da campanha. Idempotente.

alter table public.partner_campaigns
  add column if not exists theme text not null default 'forest';
alter table public.partner_campaigns
  add column if not exists layout text not null default 'overlay';

-- Valores permitidos (TEXT+CHECK, padrão do projeto — extensível no futuro).
do $$
begin
  alter table public.partner_campaigns
    add constraint partner_campaigns_theme_check
    check (theme in ('forest', 'sunset', 'ocean', 'berry', 'midnight', 'lime'));
...
begin
  alter table public.partner_campaigns
    add constraint partner_campaigns_layout_check
    check (layout in ('overlay', 'split', 'solid'));
exception when duplicate_object then null;
end $$;

-- RPC de upsert recriada incluindo _theme e _layout.
...
begin
  ...
  if _id is null then
    ...
    update public.partner_campaigns set
      ...
      theme = coalesce(_theme, 'forest'),
      layout = coalesce(_layout, 'overlay'),
      updated_at = now()
    where id = _id
    returning * into _row;
  end if;
